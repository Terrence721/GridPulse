...
| Date | What |
| - | - |
| 2026-09-11 | `git init`; `.gitignore` (.NET/Aspire/Node/Angular/Docker); MIT `LICENSE` (Terrence Daniels); `README.md` describing the architecture and 6-phase build plan; default branch renamed `master` → `main`. [#1](https://github.com/Terrence721/GridPulse/issues/1) |
...
| Date | What |
| - | - |
| 2026-09-11 | Yarn Berry pinned to 4.18.0 via Corepack (`packageManager` in `package.json`); `.yarnrc.yml` sets `nodeLinker: node-modules` (Angular-tooling compatibility) plus an empty `catalog`/`catalogs` scaffold, populated as each JS/TS package (Notification Service, Angular dashboard) actually lands. [#2](https://github.com/Terrence721/GridPulse/issues/2) |
...
| 10 | Phase 4 — Angular/NgRx dashboard + BFF gateway | Backlog — [#13](https://github.com/Terrence721/GridPulse/issues/13) |
